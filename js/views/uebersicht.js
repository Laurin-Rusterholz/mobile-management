// ============================================================================
//  Übersicht — konfigurierbare Karten (Daily Briefing, Aufgaben, Projekte, …)
//  Das Karten-Dashboard; der Homebildschirm selbst ist der Springboard.
// ============================================================================
...
function openConfig() {
  openSheet({ title: 'Übersicht anpassen', size: 'half', body: configBody(), onClose: () => navigate('uebersicht') });
}

export default {
  title: 'Übersicht', icon: '📋',
  render() {
    const c = cfg();
    const cards = orderedKeys(c).filter(k => !c.hidden.includes(k)).map(renderCard).filter(Boolean).join('');
    return `<div class="pad">
      <div class="home-top">
        <div class="home-greet">Übersicht</div>
